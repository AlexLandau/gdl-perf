...
public enum JavaEngineType {
    ...
    private static PerfTestRunnable getRuleEnginePerfTestRunnable(
        ...
        return new PerfTestRunnable() {
            ...
            private <M, S extends RuleEngineState<M, S>>
...
//            StateMachine sm = smf.buildInitializedForGame(game);
                ...
                while (true) {
                    ...
                    while (!re.isTerminal(curState)) {
                        ...
                        for (int r = 0; r < re.getNumRoles(); r++) {
                            ...
                            recorder.recordLegalMoves(toMoves(re.getTranslator().getGdlMoves(legalMoves)));
                            jointMove.add(pickOneAtRandom(legalMoves));
                        }
                        recorder.recordChosenJointMove(toMoves(re.getTranslator().getGdlMoves(jointMove)));
                        curState = re.getNextState(curState, jointMove);
                        stateChangesSoFar++;
                    }
                    recorder.recordTerminality(true);
                    recorder.recordGoalValues(re.getGoals(curState).asList());
                    //Do we end here?
            ...
            private List<Move> toMoves(List<GdlTerm> gdlMoves) {
                return gdlMoves.stream()
                            .map(Move::new)
...
//        BlockingQueue<GameActionMessage> messages = GameActionParser.convert(in);
    ...
    public static List<Role> toRoles(ExpList expRoles) {
        ...
        for (int r = 0; r < expRoles.size(); r++) {
            ...
            roles.add(new Role(expRole.toString()));
        }
        return roles;
    }
